...
# 1. 데이터 불러오기
path = "McDonald_cleaned_reviews_numeric_rating.csv"  # PyCharm 실행 디렉토리에 있는 경우
...
# 모델 저장
print("\n모델 저장 중...")
save_path = "mobilebert_custom_model_mcdonald"
model.save_pretrained(save_path)
print("모델 저장 완료!")

# 전체, 학습, 검증 데이터 수 출력
total_data_count = len(df)
train_data_count = len(train_labels)
val_data_count = len(validation_labels)

print("\n==== 데이터 개수 요약 ====")
print(f"전체 데이터 수: {total_data_count:,}건")
print(f"학습 데이터 수: {train_data_count:,}건")
print(f"검증 데이터 수: {val_data_count:,}건")
