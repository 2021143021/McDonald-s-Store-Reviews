...
총 데이터 수가 28,570건이며, 이 중 10%~20%를 추출하여 학습 데이터를 구성하였다.  
학습 데이터 구축의 핵심은 전체 데이터에서 일부를 그저 랜덤하게 추출하는 것이 아니라, **전체 데이터의 분포를 고려한 층화 추출(stratified sampling)을 이용**하여 일반화 가능성을 높이는 것이다.
...
| 추출 비율 | 추출 방식       | 긍정 리뷰 수 | 부정 리뷰 수 | 총합     |
|-----------|------------------|--------------|--------------|----------|
| 10%       | 분포 유지 추출   | 1,593건      | 1,264건      | 2,857건  |
| 10%       | 균등 분포 추출   | 1,428건      | 1,428건      | 2,856건  |
| 20%       | 분포 유지 추출   | 3,187건      | 2,527건      | 5,714건  |
| 20%       | 균등 분포 추출   | 2,857건      | 2,857건      | 5,714건  |

- 추가 고려 사항으로는 **시간대별**, **매장 위치별**, **평점별 세부 필터링** 등을 적용할 수 있다.
...
첫 번째 학습 데이터는 전체 리뷰 데이터에서 **긍정 1,000건**과 **부정 1,000건**을 추출하여 총 **2,000건**으로 구성하였다.  
두 번째 학습 데이터는 전체 감정 분포(긍정 55.8%, 부정 44.2%)를 유지하여 **10% 비율(약 2,857건)**로 추출하였으며,  
긍정 1,593건과 부정 1,264건으로 구성하였다.

---

* 각 학습 데이터별 감정 라벨 수

|                    | **첫 번째 학습 데이터** |           | **두 번째 학습 데이터** |           |
|--------------------|------------------------|-----------|------------------------|-----------|
|                    | 긍정 리뷰 수           | 부정 리뷰 수 | 긍정 리뷰 수           | 부정 리뷰 수 |
| **데이터 수(건)**  | 1,000                  | 1,000     | 1,593                  | 1,264     |
...
- 첫 번째 데이터는 **긍·부정 비율을 1:1로 균형** 있게 맞추어 모델 편향을 최소화한다.  
- 두 번째 데이터는 **원본 분포(55.8% vs 44.2%)를 유지**하여 실제 서비스 환경을 반영한다.  
- 이 두 방식을 비교 분석하여 **모델 일반화 성능**과 **현실 반영 성능**을 모두 평가한다.
