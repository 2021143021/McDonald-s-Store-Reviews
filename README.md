...
본 프로젝트에서는 전체 약 28,570건의 맥도날드 리뷰 데이터를 기반으로, 감정 분류 모델 학습에 활용할 데이터를 10~20% 비율로 추출하였다.  
단순 무작위 샘플링이 아닌 **감정 비율 및 분포를 고려한 학습 데이터 구축 방식**을 적용하였다.

---

### ✅ Case 1. 실제 감정 비율 반영

- 전체 데이터의 감정 비율(긍정 약 55.4%, 부정 약 44.6%)을 그대로 반영하여 학습 데이터를 구성하였다.
- 실제 운영 환경과 유사한 조건을 모의하여, 현실적인 모델 성능 확인이 가능하다.

| 구성 방식 | 긍정 리뷰 | 부정 리뷰 | 총합 |
|-----------|-----------|-----------|------|
| 실제 분포 기준 10% | 1,580건 | 1,270건 | 2,850건 |

---

### ⚖️ Case 2. 감정 균형 기반 구성

- 긍정과 부정 비율을 1:1로 맞춰 10% 데이터를 균형 있게 추출하였다.
- 특정 감정에 치우친 분류기 편향을 방지하고자 한 접근 방식이다.

| 구성 방식 | 긍정 리뷰 | 부정 리뷰 | 총합 |
|-----------|-----------|-----------|------|
| 균형 샘플링 10% | 1,425건 | 1,425건 | 2,850건 |

---

### 🧩 Case 3. 전체 데이터 기반 분할 학습

- 전체 데이터를 학습과 검증용으로 8:2 비율로 분할하여 모델을 학습하였다.
- 총 28,570건을 모두 사용하였으며, 가장 높은 정확도를 기록하였다.
...
#### 가상의 예: 총 데이터 50,000건 중 긍정 70%(35,000건), 부정 30%(15,000건)일 때

| 추출 방식 | 긍정 리뷰 | 부정 리뷰 | 설명 |
|-----------|-----------|-----------|------|
| (1안) 실제 분포 유지 | 3,500건 | 1,500건 | 10% 샘플링, 실제 비율 반영 |
| (2안) 균형 유지 | 2,500건 | 2,500건 | 10% 샘플링, 감정 균형 유지 |

- 실제 프로젝트에 따라 감정의 불균형을 반영하거나, 균형 조정이 필요한 상황에 맞춰 선택적으로 적용 가능하다.
- 추가로 시점별 분포, 분류 목적에 따라 세부적인 기준 설정도 가능하다.
...
<p align="center">
  <img width="600" src="https://github.com/rlagnldlf/Capstone_Project/assets/136410965/4bf86b33-d539-499b-a06f-e09b6416207c" alt="학습 데이터 분포 그래프">
</p>
...
- 전체 28,570건 중 10% 비율을 기준으로 **실제 분포 기반**과 **균형 기반** 두 가지 학습 데이터를 구성하였다.
- 더불어, 전체 데이터를 학습에 사용한 경우에는 정확도 향상이 뚜렷하게 나타났다.
- 모델 성능과 일반화 가능성을 균형 있게 확보하기 위해, 다양한 방식의 학습 데이터 구성 전략을 함께 실험하였다.
