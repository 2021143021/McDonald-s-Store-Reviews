...
본 프로젝트에서는 맥도날드 리뷰 데이터셋 총 **28,570건** 중 일부를 추출하여 모델 학습에 활용하였다.  
학습 데이터의 구성 방식에 따라 두 가지 실험을 진행하였다.

- **첫 번째 학습 데이터**는 전체 데이터에서 **긍정 1,000건 / 부정 1,000건**을 랜덤 추출하여 **총 2,000건**으로 구성되었다.
- **두 번째 학습 데이터**는 전체 데이터 중 **80%를 학습**, **20%를 검증용 데이터**로 분리하여 학습을 수행하였다.

---

### 🔹 데이터 구성

| 데이터 구성 방식               | 긍정 리뷰 수 | 부정 리뷰 수 | 총합  |
|------------------------------|--------------|--------------|-------|
| 첫 번째 학습 데이터 (균형)     | 1,000         | 1,000         | 2,000 |
| 두 번째 학습 데이터 (전체 분할) | 약 15,800     | 약 12,700     | 28,570 (전체) |

- 첫 번째 데이터는 긍/부정 비율을 균등하게 맞춘 **균형 데이터**이며,
- 두 번째는 전체 데이터를 활용하되, 자연적인 감정 분포(약 **긍정 56%, 부정 44%**)를 반영한 방식이다.
...
<p align="center">
  <img width="600" src="https://github.com/사용자아이디/저장소이름/assets/이미지경로예시.png" alt="감정 비율 그래프">
</p>

- 위 그래프는 전체 데이터셋의 **긍정/부정 비율 분포**를 시각화한 결과로,
  - 긍정 리뷰: **약 55.8%**
  - 부정 리뷰: **약 44.2%**  
- 이는 실제 고객 리뷰에서 부정적인 피드백도 적지 않게 나타남을 시사한다.
...
| 추출안             | 데이터 수 | 구성                       | 학습 목적               |
|-------------------|-----------|----------------------------|------------------------|
| 1안 (균형 샘플링)  | 2,000건    | 긍정 1,000 / 부정 1,000    | 빠른 테스트 및 비교 분석 |
| 2안 (전체 분할)    | 28,570건   | 약 80:20 비율로 분리        | 최종 학습용, 일반화 기대 |

---

이러한 학습 데이터 구성은 실험 목적과 일반화 성능을 함께 고려한 설계이며,  
다음 섹션에서는 실제 학습 결과와 성능 평가에 대해 다룬다.
