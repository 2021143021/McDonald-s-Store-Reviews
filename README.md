...
향후에는 햄버거 외에도 다양한 프랜차이즈 브랜드로 확장하거나, 멀티모달(이미지+리뷰) 감정 분석까지 시도해보고 싶다.
...
저는 **햄버거를 제일 좋아하며**, 특히 **맥도날드를 일주일에 여러 번 방문할 정도로 자주 먹는 편**입니다.  
실제로 다양한 신제품이 출시될 때마다 관심 있게 살펴보고, 맥도날드 앱 리뷰를 읽는 습관도 있었기 때문에  
**개인적인 흥미와 실생활에서의 친숙함**으로 인해 "**맥도날드 리뷰 감정 분석**"이라는 주제에 자연스럽게 끌렸습니다.  
단순한 관심을 넘어서 **리뷰 감정과 평점 간의 관계를 인공지능으로 분석해보고 싶다**는 동기가 생겼고,  
그 관심이 이번 프로젝트로 이어지게 되었습니다.
...
- **감정 분석의 유용성**  
  숫자 평점만으로는 제품이나 서비스에 대한 진짜 감정을 파악하기 어렵다는 점을 다시 한 번 느꼈습니다.  
  같은 4점 리뷰라도 누군가는 아쉬움을 표현하고, 누군가는 극찬을 하는 등 **점수와 텍스트 간의 간극**이 분명 존재했습니다.  
  이 프로젝트를 통해 **텍스트 기반 감정 분석 모델의 중요성과 실용성**을 체감했습니다.

- **모델 선택과 튜닝의 중요성**  
  단순히 모델을 학습시키는 것만으로는 좋은 결과를 얻을 수 없었습니다.  
  학습률 설정, 데이터 전처리, 에폭 조절 등 **세부적인 설정값 하나하나가 모델의 성능에 큰 영향을 미친다는 점**을 깨달았고,  
  특히 **MobileBERT** 모델을 통해 적은 리소스로도 높은 정확도를 낼 수 있다는 점은 매우 흥미로웠습니다.

- **실제 데이터를 다루는 어려움과 보람**  
  데이터를 직접 탐색하고 정제하며 "어떤 문장이 긍정이고 부정인가"를 모델에 학습시키는 과정은 **쉽지 않았지만**,  
  그만큼 **모델이 정확하게 예측하는 모습을 확인할 때 큰 보람**을 느꼈습니다.

---

### ❗ 아쉬웠던 점

- 초기에 감정 레이블 기준을 정하는 과정에서 일부 **중립적인 문장을 긍정/부정으로 구분하기 어려웠습니다.**
- 실제 앱 리뷰처럼 **긴 문장, 복합 문장**이 포함된 경우에는 모델이 **의미를 일부 놓치는 경향**도 보였습니다.
- 추후에는 **다층적 감정 레이블링(긍정/중립/부정)**이나, **문맥 기반 모델(BERT 계열 확장)**을 시도해 개선해보고 싶습니다.

---

### 🧠 개인적인 성찰

비록 제 코딩 실력과 지식이 부족해서 **완성도 면에서는 아쉬움이 있었지만**,  
이번 프로젝트를 통해 **몰랐던 다양한 기술과 코딩 역량**, 그리고 **README 문서 작성법**까지 새롭게 배울 수 있어서 매우 뜻깊은 경험이었습니다.  

특히, 실력이나 지식이 부족하더라도 내가 평소에 가장 좋아하는 브랜드인 '맥도날드'라는 관심사를 주제로  
**끝까지 포기하지 않고 꾸준히 나아감으로써** 프로젝트를 마무리할 수 있었다는 점에서 **매우 뿌듯하고 속이 후련합니다.**  
앞으로도 이러한 경험을 바탕으로 **더 나은 프로젝트를 해낼 수 있다는 자신감**도 얻게 되었습니다.

---

### 🎓 배운 점 요약

- **자연어 처리(NLP)** 및 **감정 분류 모델 구조**에 대한 실습 경험
- 학습/검증/테스트 데이터의 분리 및 **모델 성능 평가 지표 활용**
- **matplotlib를 활용한 시각화**, 정량적 결과 표현 능력 향상
- 프로젝트를 **README.md 형식으로 정리하는 능력 강화**

---

이 프로젝트는 단순한 음식 리뷰 분석을 넘어,  
**AI가 실제 서비스 리뷰를 어떻게 이해할 수 있는지를 탐색한 의미 있는 경험**이었습니다.  
향후에는 햄버거 외에도 다양한 프랜차이즈 브랜드로 확장하거나,  
**멀티모달(이미지 + 리뷰 텍스트) 감정 분석**까지 시도해보고 싶습니다.
